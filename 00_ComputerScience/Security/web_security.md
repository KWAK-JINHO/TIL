...
- 출력값 이스케이프 (Escape): 서버에서 사용자 입력값을 화면에 출력하기 전에 `<`와 `>` 같은 스크립트 실행에 사용되는 특수문자를 단순 문자열로 치환하여
- (`&lt;`, `&gt;`) 브라우저가 스크립트로 인식하지 못하게 한다.

# TLS/SSL

SSL (Secure Sockets Layer) 과 TLS (Transport Layer Security)은 같은 기술이며 명칭만 달라진것. 현재는 TLS라고 부르며 인터넷 통신을 암호화하는데 사용되는 프로토콜. HTTP에서 TLS를 보안 계층에 적용하여 HTTPS라고 부른다. 클라이언트가 로그인 또는 회원가입 당시 서버로 패스워드를 암호화해서 보내지 않아도 되는 이유는 HTTPS 설정으로 보낼때 암호화 되기 때문. 그래도 DB에 저장할때는 암호화해서 저장해야 한다.